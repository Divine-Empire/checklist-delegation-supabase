                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                    ...
                    <div className="flex border border-purple-200 rounded-md overflow-hidden self-start">
                        <button
                            ...
                            onClick={() => { setActiveTab('checklist'); onTabChange('checklist'); }}
                        >
                            Checklist
                        </button>
                        <button
                            ...
                            onClick={() => { setActiveTab('delegation'); onTabChange('delegation'); }}
                        >
                            Delegation
                        </button>
                    </div>
                    ...
                    <div className="flex gap-2">
                        ...
                        <div className="relative" ref={nameFilterRef}>
                            <div className="flex items-center gap-2">
                                <div className="relative">
                                    ...
                                    {nameFilter && (
                                        <button onClick={() => { setNameFilter(''); onNameSelect(''); }} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400">
                                            <X size={16} />
                                        </button>
                                    )}
                                </div>
                                <button onClick={() => toggleDropdown('name')} className="px-3 py-2 border border-purple-200 rounded-md bg-white hover:bg-gray-50">
                            ...
                            {dropdownOpen.name && (
                                <div className="absolute z-50 mt-1 w-56 rounded-md bg-white shadow-lg border border-gray-200 max-h-60 overflow-auto top-full right-0">
                                    <button onClick={() => { setNameFilter(''); onNameSelect(''); }} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100">All Names</button>
                                    {allNames.map(name => (
                                        <button key={name} onClick={() => { setNameFilter(name); onNameSelect(name); }} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100">
                                            {name}
                                        </button>
                                    ))}
                                </div>
                        ...
                        <div className="relative" ref={freqFilterRef}>
                            <button onClick={() => toggleDropdown('frequency')} className="flex items-center gap-2 px-3 py-2 border border-purple-200 rounded-md bg-white text-sm text-gray-700 hover:bg-gray-50">
                                ...
                                <ChevronDown size={16} className={dropdownOpen.frequency ? 'rotate-180' : ''} />
                            </button>
                            {dropdownOpen.frequency && (
                                <div className="absolute z-50 mt-1 w-56 rounded-md bg-white shadow-lg border border-gray-200 max-h-60 overflow-auto">
                                    <button onClick={() => { setFreqFilter(''); closeDropdowns(); }} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100">All</button>
                                    {allFrequencies.map(freq => (
                                        <button key={freq} onClick={() => { setFreqFilter(freq); closeDropdowns(); }} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100">
                                            {freq}
                                        </button>
                                    ))}
                                </div>
                        ...
                        {/* Delete Button */}
                        {userRole === 'admin' && selectedTasks.length > 0 && activeTab === 'checklist' && (
                            <button
