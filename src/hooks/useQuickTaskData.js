...
export const useQuickTaskData = () => {
    ...
    const {
        quickTask, loading, delegationTasks, users,
        checklistPage, checklistHasMore,
        delegationPage, delegationHasMore
    } = useSelector((state) => state.quickTask);

    const {
        activeTab, nameFilter, searchTerm, freqFilter, sortConfig,
        setSortConfig, toggleDropdown, closeDropdowns
    } = useQuickTaskUIStore();
    ...
    const filteredChecklistTasks = useMemo(() => {
        let filtered = quickTask.filter(task => {
            ...
            const searchPass = !searchTerm || task.task_description?.toLowerCase().includes(searchTerm.toLowerCase());
            return freqPass && searchPass;
        });
    ...
    }, [loading, activeTab, checklistPage, checklistHasMore, delegationPage, delegationHasMore, nameFilter, dispatch]);
    ...
    const handleNameFilterSelect = (name) => {
        if (activeTab === 'checklist') {
            dispatch(resetChecklistPagination());
            dispatch(uniqueChecklistTaskData({ page: 0, pageSize: 50, nameFilter: name }));
        } else {
            dispatch(resetDelegationPagination());
            dispatch(uniqueDelegationTaskData({ page: 0, pageSize: 50, nameFilter: name }));
        }
        closeDropdowns();
    };
